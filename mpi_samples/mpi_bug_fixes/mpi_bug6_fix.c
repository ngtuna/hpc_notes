...
//    MPI_Irecv(&buf, 1, MPI_INT, src, tag1, COMM, &reqs[offset+1]);
    MPI_Isend(&rank, 1, MPI_INT, dest, tag1, COMM, &reqs[i]);
    MPI_Irecv(&buf, 1, MPI_INT, src, tag1, COMM, &reqs[REPS+i]);
...
if (!(rank==2)) 
{
   printf("Task %d should call MPI_Wait\n", rank);
   MPI_Waitall(nreqs, reqs, stats);
}
else
{
printf("Task %d should not call MPI_Wait\n", rank);
}
